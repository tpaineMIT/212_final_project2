...
global canny, center, dist, d_p, l_h, u_h, l_s, u_s, l_v, u_v, bridge, URCoordArr
# Start with red
center=1
dist=1000
d_p=2
...
def callback(data):
    ...
    if circles is not None:
        ...
        for i in circles[0,:]:
            ...
            #convert screen coordinates to cartesian coordinates (still in pixel values)
            width = round(rows)
            height = round(cols)
            cameraCartx = screenCoordx - width
            cameraCarty = -screenCoordy + height

            #convert camera cartesian to measurements for (in METERS)
            ###YOOOO THESE COORDS ARE IN METERS
            URCoordx = (40.5 - 36.5) * 0.01
            URCoordy = (31 + 37) * 0.01
            URCoordz = (78) * 0.01
...
def listener():
    ...
    #URCoord = rospy.Publisher("urCoord_fromImage", URCoordArr)
