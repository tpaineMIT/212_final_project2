...
tagRetDists = [0,0,0,2.785,3.96,1.295] # index of list corresponds to tag ID, entry corresponds to approach distance
...
#				thetaDot = -yawMov * 3 # hard code for now
			...
			if elapsed_time > 3.0:
				if abs(relX) > .5: #Choose some large amount to represent having gone off-course
					print('Went off-course. Looking for tag again.')
					pointAtTag(target)
					# Need to call approachRetreat() again somewhere, but not recursively
						# Maybe not actually, since we're in a while approached==False loop
				else: # Need user help
					stopAuto = True
					print 'There is an obstacle I cannot go around.'
